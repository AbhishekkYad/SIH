...
const STORIES: StakeholderStory[] = [
  {
    ...
    quote: 'For the first time, my harvest quality is permanently linked to my name, earning fair market premiums without middlemen dilution.',
    actor: 'Ramesh Patil (Organic Farmer Cluster #402)',
    location: 'Nashik Valley, Maharashtra',
    icon: '🌾',
    story: 'The story starts in the soil. Before a single seed is sown, organic bio-fertilizer usage, GPS farm geofencing, and soil moisture levels are digitally logged. At harvest time, raw grain tonnage is cryptographically signed with the farmer’s decentralized ID (DID) and minted as the genesis asset on the ledger.',
    keyActions: [
    ...
    techProof: 'W3C DID Signature · Polygon Geofence · Zero-Knowledge Origin Proof',
  ...
  },
  {
    ...
    quote: 'We transform raw agricultural yields into certified packaged goods, preserving complete parent-child recursive lineage.',
    actor: 'Sahyadri Milling & Processing Unit #04',
    location: 'Chakan Industrial Hub, Pune',
    icon: '⚙️',
    story: 'Raw grains arriving at the mill undergo pneumatic de-stoning, multi-stage optical laser sortex cleaning, and granulation testing. The lineage engine creates an immutable parent-child transformation graph that binds incoming farm batch IDs directly to the output flour lots.',
    keyActions: [
    ...
    techProof: 'Lineage DAG Transformation · NABL Lab COA on IPFS · Mass-Balance Audit',
  ...
  },
  {
    ...
    quote: 'Every carton in our cold-storage facility is continuously monitored for humidity and temperature excursions.',
    actor: 'Central Cold Storage & Packaging Facility',
    location: 'Tathawade Logistics Hub, Pune',
    icon: '📦',
    story: 'Packaged commodities move into automated warehouses where IoT telemetry monitors ambient temperature (18°C) and relative humidity (54%). High-speed printers apply dynamic, serialized GS1 Digital Link QR codes to each individual consumer carton.',
    keyActions: [
  ...
  },
  {
    ...
    quote: 'Our refrigerated fleet streams GPS and temperature telemetry every 5 minutes along major delivery corridors.',
    actor: 'AgriTransit Cold Fleet MH-12',
    location: 'Mumbai-Pune Freight Expressway',
    icon: '🚚',
    story: 'Refrigerated trucks pick up sealed pallets for transit across urban fulfillment corridors. BLE sensors broadcast tamper-evident seal verification and temperature stability. Any thermal shock or route deviation triggers an immediate blockchain event alert.',
    keyActions: [
    ...
    techProof: 'Telemetry Stream · E-Way Bill Integration · Tamper-Evident Seals',
  ...
  },
  {
    ...
    quote: 'Inbound scanning verifies product authenticity instantly, with automatic recall lock capability built right into our barcode scanners.',
    actor: 'GreenBasket Hypermarket Bandra',
    location: 'Mumbai Retail Outlets',
    icon: '🏬',
    story: 'Supermarket receiving docks scan incoming pallets to authenticate digital provenance. Stock is mapped to store shelves with full shelf-life tracking. In the event of a verified risk incident, central systems dispatch a barcode lock to POS terminals in under 200ms.',
    keyActions: [
    ...
    techProof: 'Fabric Peer Node · POS Scanner API · Autonomous Quarantine Sync',
  ...
  },
  {
    ...
    quote: 'I scanned the QR code on the bag and instantly saw the exact farm in Nashik, harvest date, and lab safety certificate.',
    actor: 'Everyday Household Consumer',
    location: 'Point of Purchase / Kitchen Table',
    icon: '📱',
    story: 'Using any standard smartphone camera, the consumer scans the GS1 QR code on the food package. They instantly view the farm origin, farmer payout, milling date, lab safety reports, and can submit verified sensory feedback directly back into the ecosystem.',
    keyActions: [
    ...
    techProof: 'GS1 Digital Link URI · Decentralized Web Reader · Consumer Feedback Loop',
    ...
    <div className={styles.pageWrap}>
      ...
      <main className={styles.main}>
          ...
          <div className="container">
            <div className={styles.heroTop}>
              ...
              <p className={styles.pageLead}>
                Traceability is not just lines of code — it is the connected journey of real farmers, millers,
                warehouse custodians, drivers, retailers, and consumers working in unified trust.
              </p>
            </div>
              ...
              {STORIES.map((s, idx) => (
                ...
                >
                  ...
                  <div className={styles.flowNodeText}>
                    ...
                    <span className={styles.flowNodeRole}>{s.role.split('/')[0]}</span>
                  </div>
                  {idx < STORIES.length - 1 && <span className={styles.flowArrow}>→</span>}
                </button>
              ))}
            </div>
          </div>
            ...
            <div className={styles.storyCard}>
              ...
              <div className={styles.storyContent}>
                <div className={styles.storyHeader}>
                  <div className={styles.badgeRow}>
                    <span className={styles.numberBadge}>STAKEHOLDER {activeStory.number} OF 06</span>
                    <span className="chip chip--green">{activeStory.location}</span>
                  </div>
                ...
                <blockquote className={styles.quoteBox}>
                  <p className={styles.quoteText}>&ldquo;{activeStory.quote}&rdquo;</p>
                  <cite className={styles.quoteAuthor}>— {activeStory.actor}</cite>
                </blockquote>

                <div className={styles.bodyBlock}>
                  <h4 className={styles.blockHeading}>The Role in the Traceability Chain</h4>
                  <p className={styles.narrativeText}>{activeStory.story}</p>
                </div>

                <div className={styles.actionsBlock}>
                  <h4 className={styles.blockHeading}>Verifiable Key Actions Committed</h4>
                ...
                <div className={styles.techStrip}>
                  ...
                  <span className={styles.techVal}>{activeStory.techProof}</span>
                </div>
              </div>
              ...
              <div className={styles.storyVisualWrap}>
                <div className={styles.imageCard}>
                  <img src={activeStory.image} alt={activeStory.role} className={styles.storyImg} />
                  <div className={styles.imageOverlay}>
                    <span className={styles.overlayIcon}>{activeStory.icon}</span>
                    <span className={styles.overlayActor}>{activeStory.actor}</span>
                  </div>
                </div>
                ...
                <div className={styles.navigationRow}>
                  <button
                    ...
                    onClick={() => {
                      const currIdx = STORIES.findIndex((s) => s.id === activeStoryId);
                      const prevIdx = (currIdx - 1 + STORIES.length) % STORIES.length;
        ...
        <section className={styles.gridSection}>
          <div className="container">
            <h3 className={styles.gridHeading}>All 6 Pillars of the FoodTrace Ecosystem</h3>
