...
export default function OneFoodPage() {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setUploadNotice(`✓ Successfully updated ${activeItem.category} image with "${file.name}"`);
  ...
  const handleClearActiveImage = () => {
    setGallery((prev) =>
      prev.map((item) => (item.id === activeItemId ? { ...item, src: '' } : item))
    );
    setUploadNotice(`Cleared image for ${activeItem.category}. Empty state rendered.`);
    setTimeout(() => setUploadNotice(null), 3000);
  };
          ...
          <div className="container">
            <div className={styles.heroTop}>
              <div className={styles.badgeRow}>
                <span className="chip chip--green">● VERIFIED GENESIS BATCH</span>
                <span className={styles.batchTag}>ID: WF-2026-0815</span>
              </div>
              <h1 className={styles.pageTitle}>
                Single Product Provenance Explorer: <strong>Organic Sharbati Wheat Flour</strong>
              </h1>
              <p className={styles.pageLead}>
                Inspect photographic evidence, laboratory certificates, and cryptographic sensor logs committed
                across each handover stage for Batch <strong>WF-2026-0815</strong>.
              </p>
            </div>

            {/* Quick Crop Selector Pills */}
            <div className={styles.cropSelector}>
              <span className={styles.cropSelectorLabel}>SWITCH PRODUCT DEMO:</span>
              {['Wheat', 'Oats', 'Peas', 'Chickpeas', 'Sunflower', 'Flaxseed'].map((crop) => (
                <button
                  key={crop}
                  type="button"
            ...
            <div className={styles.workspaceGrid}>
              ...
              <div className={styles.controlPanel}>
                <div className={styles.panelHead}>
                  <h3 className={styles.panelTitle}>Supply Chain Visual Assets</h3>
                  ...
                  {gallery.map((item) => (
                    ...
                    >
                      ...
                      <div className={styles.slotInfo}>
                        ...
                        <span className={styles.slotTime}>{item.timestamp}</span>
                      </div>
                ...
                <div className={styles.actionBox}>
                  <span className={styles.actionBoxLabel}>EVIDENCE MEDIA CONTROLS</span>
                ...
                <div className={styles.previewCard}>
                  ...
                  <div className={styles.mediaContainer}>
                    {activeItem.src ? (
                      ...
                      <div className={styles.emptyState}>
                        ...
                        <h4 className={styles.emptyTitle}>No image uploaded for {activeItem.category}</h4>
                        <p className={styles.emptyDesc}>
                          Click the &ldquo;Upload Image&rdquo; button on the left to attach verified photographic proof.
                        </p>
                        <button
                          type="button"
                          className="btn btn--grass"
                  ...
                  <div className={styles.metadataGrid}>
                    <div className={styles.metaItem}>
                      <span className={styles.metaKey}>CAPTURED TIMESTAMP:</span>
                    ...
                    <div className={styles.metaItem}>
                      <span className={styles.metaKey}>VERIFICATION SUMMARY:</span>
        ...
        <section className={styles.specsSection}>
          <div className="container">
            <div className={styles.specsCard}>
              <h3 className={styles.specsTitle}>Verified Laboratory Analysis &amp; Origin Metrics</h3>
              <div className={styles.specsGrid}>
                <div className={styles.specBox}>
                  <span className={styles.specKey}>Pesticide Residue Screen</span>
                  <span className={styles.specVal} style={{ color: 'var(--color-grass-500)' }}>0.00 PPM (100% Organic)</span>
                  <span className={styles.specSub}>Tested against 180 Organophosphates</span>
                </div>
                <div className={styles.specBox}>
                  <span className={styles.specKey}>Moisture Content</span>
                  <span className={styles.specVal}>11.8% Standard</span>
                  <span className={styles.specSub}>Safe storage moisture &lt; 12.5%</span>
                </div>
                <div className={styles.specBox}>
                  <span className={styles.specKey}>Sortex Optical Purity</span>
                  <span className={styles.specVal} style={{ color: 'var(--color-grass-500)' }}>99.92% Clean</span>
                  <span className={styles.specSub}>Triple-stage laser sortex</span>
                </div>
                <div className={styles.specBox}>
                  <span className={styles.specKey}>Farmer Revenue Share</span>
                  <span className={styles.specVal} style={{ color: 'var(--color-grass-500)' }}>72.4% Direct Payout</span>
                  <span className={styles.specSub}>Smart contract escrow settled</span>
                </div>
              </div>
            </div>
          </div>
