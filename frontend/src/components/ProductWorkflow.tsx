...
const STAGES: WorkflowStage[] = [
  {
    ...
    qualityMetrics: [
      ...
      { label: 'Chemical Residue', value: '0.00 PPM (100% Organic)', pass: true },
    ...
    actionDetails: 'Harvested 12,500 KG Sharbati Wheat. Harvest telemetry signed with W3C DID private key and minted as parent asset on Hyperledger ledger.',
    icon: '🌾',
  },
  {
    id: 'collection',
    ...
    qualityMetrics: [
      ...
      { label: 'Weighbridge Certificate', value: 'ELECTRONIC_SEAL_VALID', pass: true },
    ],
    verificationHash: '0x3c11...7a4f90',
    prevStage: 'Farmer Origin & Harvest',
    nextStage: 'Sortex Milling & Processing',
    actionDetails: 'Mandate digital weighment reconciliation against farmer manifest. Dispatched via dedicated sealed trailer to Pune processing facility.',
    icon: '🏢',
  },
  {
    id: 'processing',
    ...
    statusColor: 'green',
    qualityMetrics: [
      { label: 'Optical Sortex Purity', value: '99.92% Defect-Free', pass: true },
    ...
    actionDetails: 'Multi-stage Sortex optical separation and pneumatic de-stoning. Lineage transformation linked parent LOT-NSK-2026-W402 to output batch PROC-PUN-0914-A.',
    icon: '⚙️',
  },
  {
    id: 'quality',
    ...
    actionDetails: 'Comprehensive microbiological culture and spectrographic pesticide screening. Digital COA (Certificate of Analysis) uploaded with IPFS hash.',
    icon: '🔬',
  },
  {
    id: 'warehouse',
    ...
    qualityMetrics: [
      ...
      { label: 'Dual-QR Generation', value: '4,280 Unique Item Serialized', pass: true },
    ],
    verificationHash: '0x1a88...ff44aa',
    prevStage: 'Lab Quality & Safety Check',
    nextStage: 'Cold-Chain Freight & Logistics',
    actionDetails: 'Automated nitrogen-flushed packaging into 1KG and 5KG consumer packs. Dual-QR tamper codes minted with GS1 Digital Link URIs.',
    icon: '📦',
  },
  {
    id: 'distributor',
    ...
    qualityMetrics: [
      ...
      { label: 'Route GPS Compliance', value: 'Zero Geofence Deviation', pass: true },
      { label: 'E-Way Bill Signature', value: 'GST-EWAY-2026-9021', pass: true },
    ],
    verificationHash: '0x99dd...01bc55',
    prevStage: 'Smart Storage & Packaging',
    nextStage: 'Retail Supermarket POS',
    actionDetails: 'Continuous IoT Bluetooth BLE sensor streaming telemetry every 5 minutes. No thermal shock or seal breakage recorded during 148 km journey.',
    icon: '🚚',
  },
  {
    id: 'retailer',
    ...
    statusColor: 'blue',
    qualityMetrics: [
      { label: 'Carton Seal Audit', value: 'Intact Cryptographic Seal', pass: true },
      { label: 'Shelf Stocking Date', value: '2026-08-13 (Fresh Pack)', pass: true },
      { label: 'POS Terminal Sync', value: 'Fabric Peer Node Verified', pass: true },
    ],
    verificationHash: '0x22ee...8831ba',
    prevStage: 'Cold-Chain Freight & Logistics',
    nextStage: 'Consumer QR Verification',
    actionDetails: 'Inbound scanning automated at receiving dock. Barcode linked directly to store inventory system with immediate POS lock capability on alert.',
    icon: '🏬',
  },
  {
    id: 'consumer',
    ...
    statusColor: 'green',
    qualityMetrics: [
      { label: 'QR Scan Validation', value: 'Genuine & Authentic (1st Scan)', pass: true },
      { label: 'Farm Origin Certified', value: '100% Organic Nashik Wheat', pass: true },
    ...
    actionDetails: 'Consumer scans GS1 Digital Link QR on packaging. Full 8-stage provenance timeline, farmer payout, and feedback submission interface presented.',
    icon: '📱',
  },
];
      ...
      <div className="container">
        ...
        <div className={styles.header}>
          ...
          <p className={styles.lead}>
            Follow the live cryptographic custody trail of Batch <strong>WF-2026-0815</strong> (Sharbati Organic Wheat) across all 8 supply chain nodes in real time.
          </p>
        </div>
              ...
              return (
                ...
                >
                  ...
                  <div className={styles.nodeText}>
                    <span className={styles.nodeName}>{st.name.split('&')[0]}</span>
                    <span className={styles.nodeRole}>{st.role}</span>
                  </div>
                  {idx < STAGES.length - 1 && <div className={styles.nodeConnector} />}
                </div>
              );
            })}
          </div>
        ...
        <div className={styles.dashboardCard}>
          ...
          <div className={styles.dashHeader}>
            <div className={styles.dashHeaderLeft}>
              <div className={styles.stageNumberBadge}>STAGE {selectedStage.stepNumber} OF 08</div>
              <h3 className={styles.dashTitle}>
                <span className={styles.dashEmoji}>{selectedStage.icon}</span> {selectedStage.name}
              </h3>
            </div>
          ...
          <div className={styles.dashGrid}>
            ...
            <div className={styles.infoCol}>
              <div className={styles.infoBlock}>
                <span className={styles.infoLabel}>STAKEHOLDER &amp; IDENTITY</span>
                <span className={styles.infoValueMain}>{selectedStage.stakeholder}</span>
                <span className={styles.didBadge}>{selectedStage.stakeholderDid}</span>
              </div>

              <div className={styles.twoColRow}>
                <div className={styles.infoBlock}>
                  <span className={styles.infoLabel}>LOCATION / GPS</span>
                  <span className={styles.infoValue}>{selectedStage.location}</span>
                </div>
                <div className={styles.infoBlock}>
                  <span className={styles.infoLabel}>TIMESTAMP (UTC/IST)</span>
                  <span className={styles.infoValue}>{selectedStage.timestamp}</span>
                </div>
              </div>

              <div className={styles.infoBlock}>
                <span className={styles.infoLabel}>OPERATIONAL ACTION RECORDED</span>
                <p className={styles.actionText}>{selectedStage.actionDetails}</p>
              </div>
              ...
              <div className={styles.navigationStrip}>
                <div className={styles.navBlock}>
                  ...
                  <span className={styles.navBlockVal}>{selectedStage.prevStage}</span>
                </div>
                <div className={styles.navBlock} style={{ textAlign: 'right' }}>
                  <span className={styles.navBlockLabel}>NEXT STAGE →</span>
                  <span className={styles.navBlockVal}>{selectedStage.nextStage}</span>
                </div>
              </div>
            </div>
            ...
            <div className={styles.auditCol}>
              <span className={styles.infoLabel}>QUALITY / VERIFICATION PARAMETERS</span>
              ...
              <div className={styles.proofCard}>
                <div className={styles.proofHeader}>
                  <span className={styles.proofLabel}>ON-CHAIN VERIFICATION PROOF</span>
                  <span className={styles.proofBadge}>FABRIC BLOCK #18,402</span>
                </div>
                <div className={styles.proofHashRow}>
                  <span className={styles.hashLabel}>TX HASH:</span>
                  <code className={styles.hashCode}>{selectedStage.verificationHash}</code>
                </div>
                <p className={styles.proofNotice}>
                  Cryptographically sealed with ECDSA secp256k1 signature and verified by multi-organization consensus.
                </p>
              </div>
            </div>
          </div>
