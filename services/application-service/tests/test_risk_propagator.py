...
async def test_risk_propagation_and_recall(async_client, admin_headers):
    # 1. Propagate Risk
    prop_payload = {
        "source_batch_id": "batch-apple-001-raw",
    ...
    assert risk_data["source_batch_id"] == "batch-apple-001-raw"
    assert "affected_child_batches" in risk_data

    # 2. Block Batch
    block_res = await async_client.post("/api/v1/recall/block", json={"batch_id": "batch-apple-001-packaged", "reason": "Contamination hazard"}, headers=admin_headers)
    assert block_res.status_code == 200

    # 3. Create Recall Action
    recall_payload = {
        "reason": "Safety concern",
        "affected_batch_ids": ["batch-apple-001-packaged"],
